...
import { BellIcon, Building2, CalendarCheck, ChevronDown, ChevronLeft, ChevronRight, ClipboardListIcon, Home, LogOut, Menu, MessageSquareDiff, ScrollText, Search } from "lucide-react";
      ...
      >
        <div className="flex flex-col items-center justify-center">
          ...
          >
            ...
            <Link
              href={"dashboard"}
              className={cn(path.includes("/dashboard") || path.startsWith("/dashboard") ? "text-[#F0EAE9] bg-[#99582A]" : "",
            ...
            <div className="pt-4">
              {
                sidebarOpen && <span className="text-gray-500 uppercase font-light">
                  Management
                </span>
              }
              ...
              <div className="flex flex-col mt-2 ">
                {sideBarItems.map((item, index) => (
                  <Link
                    ...
                    href={item.to}
                    className={cn(path.includes(item.to) || path.startsWith(item.to) ? "text-[#F0EAE9] bg-[#99582A]" : "",
                      "py-2 px-3 rounded-2xl w-fit text-center flex items-center gap-2")}
                  >
                    {item.icon}
                    {sidebarOpen && <p className="whitespace-nowrap">{item.title}</p>}
                  </Link>
                ))}
              </div>
            </div>
